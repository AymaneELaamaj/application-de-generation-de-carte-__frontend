...
@Component({
  selector: 'app-nav-temara',
  templateUrl: './nav-temara.component.html',
  styleUrl: './nav-temara.component.css'
})
export class NavTemaraComponent implements OnInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  progress: number = 0;
  ...
  onUpload(): void {
    ...
    if (this.selectedFile) {
      this.redalservice.uploadtemara(this.selectedFile).subscribe({
        next: (event: any) => {
  ...
  logout(){ console.log('logout succes')
