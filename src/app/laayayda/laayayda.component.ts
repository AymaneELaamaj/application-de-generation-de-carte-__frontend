...
@Component({
  selector: 'app-laayayda',
  templateUrl: './laayayda.component.html',
  styleUrl: './laayayda.component.css'
})
export class LaayaydaComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  ...
  private loadData(L: any): void {
    this.redalservice.getlaayayda().subscribe(
      (data: any[]) => {
        if (this.map) {
