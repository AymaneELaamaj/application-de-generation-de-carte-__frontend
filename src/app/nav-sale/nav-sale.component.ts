...
@Component({
  selector: 'app-nav-sale',
  templateUrl: './nav-sale.component.html',
  styleUrl: './nav-sale.component.css'
})
export class NavSaleComponent implements OnInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  progress: number = 0;
  ...
  onUpload(): void {
    ...
    if (this.selectedFile) {
      this.redalservice.uploadsale(this.selectedFile).subscribe({
        next: (event: any) => {
