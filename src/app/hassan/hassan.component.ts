...
@Component({
  ...
  styleUrls: ['./hassan.component.css'] // Changement de `styleUrl` à `styleUrls`
})
export class HassanComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  ...
  private loadData(L: any): void {
    this.redalservice.getHassan().subscribe(
      (data: any[]) => {
        if (this.map) {
