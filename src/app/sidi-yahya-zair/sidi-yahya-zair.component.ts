...
@Component({
  selector: 'app-sidi-yahya-zair',
  templateUrl: './sidi-yahya-zair.component.html',
  styleUrl: './sidi-yahya-zair.component.css'
})
export class SidiYahyaZairComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
...
, // Centrer la carte sur votre zone d'intérêt
  ...
  private loadData(L: any): void {
    this.redalservice.getsidi_yahya_Zair().subscribe(
      (data: any[]) => {
        if (this.map) {
