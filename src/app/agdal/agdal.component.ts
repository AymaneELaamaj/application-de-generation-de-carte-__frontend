...
@Component({
  selector: 'app-agdal',
  templateUrl: './agdal.component.html',
  styleUrls: ['./agdal.component.css']
})
export class AgdalComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  ...
  private loadData(L: any): void {
    this.redalservice.getAgdal().subscribe(
      (data: any[]) => {
        if (this.map) {
