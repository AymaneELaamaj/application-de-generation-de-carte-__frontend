...
@Component({
  selector: 'app-youssoufia',
  templateUrl: './youssoufia.component.html',
  styleUrls: ['./youssoufia.component.css']
})
export class YoussoufiaComponent implements AfterViewInit {
  private map: any;
  ...
  private loadData(L: any): void {
    this.redalservice.getYoussoufia().subscribe(
      (data: any[]) => {
        if (this.map) {
          ...
          data.forEach((item: any) => {
            if (Array.isArray(item) && item.length > 6 && item[6]) {
              try {
                ...
                const dso = parseFloat(item[3]); // Assuming item[3] is the DSO value
