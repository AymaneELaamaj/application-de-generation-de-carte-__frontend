...
@Component({
  selector: 'app-tabriquet',
  templateUrl: './tabriquet.component.html',
  styleUrl: './tabriquet.component.css'
})
export class TabriquetComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  ...
  private loadData(L: any): void {
    this.redalservice.gettabriqet().subscribe(
      (data: any[]) => {
        if (this.map) {
