...
@Component({
  selector: 'app-hsaine',
  templateUrl: './hsaine.component.html',
  styleUrl: './hsaine.component.css'
})
export class HsaineComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  ...
  private loadData(L: any): void {
    this.redalservice.gethsaine().subscribe(
      (data: any[]) => {
        if (this.map) {
