...
@Component({
  selector: 'app-bettana',
  templateUrl: './bettana.component.html',
  styleUrl: './bettana.component.css'
})
export class BettanaComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  ...
  private loadData(L: any): void {
    this.redalservice.getbettana().subscribe(
      (data: any[]) => {
        if (this.map) {
