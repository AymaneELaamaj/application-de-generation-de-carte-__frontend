...
@Component({
  selector: 'app-ain-atig',
  templateUrl: './ain-atig.component.html',
  styleUrl: './ain-atig.component.css'
})
export class AinAtigComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  ...
  private loadData(L: any): void {
    this.redalservice.getAin_Atig().subscribe(
      (data: any[]) => {
        if (this.map) {
