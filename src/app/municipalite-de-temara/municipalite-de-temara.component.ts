...
@Component({
  selector: 'app-municipalite-de-temara',
  templateUrl: './municipalite-de-temara.component.html',
  styleUrl: './municipalite-de-temara.component.css'
})
export class MunicipaliteDeTemaraComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  ...
  private initMap(L: any): void {
    this.map = L.map('map', {
      center: [33.944, -6.835], // Centrer la carte sur votre zone d'intérêt
      zoom: 12,
      layers: []
    });
  ...
  private loadData(L: any): void {
    this.redalservice.getMunicipalité_de_Temara().subscribe(
      (data: any[]) => {
        if (this.map) {
