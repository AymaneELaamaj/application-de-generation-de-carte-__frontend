...
@Component({
  selector: 'app-bab-mrissa',
  templateUrl: './bab-mrissa.component.html',
  styleUrl: './bab-mrissa.component.css'
})
export class BabMrissaComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  ...
  private loadData(L: any): void {
    this.redalservice.getmrissa().subscribe(
      (data: any[]) => {
        if (this.map) {
