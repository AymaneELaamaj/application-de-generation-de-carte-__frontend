...
@Component({
  selector: 'app-bouknadel',
  templateUrl: './bouknadel.component.html',
  styleUrl: './bouknadel.component.css'
})
export class BouknadelComponent implements AfterViewInit {
  private map: any;
  selectedFile!: File;
  entitiesData: any[] = [];
  ...
  private loadData(L: any): void {
    this.redalservice.getbouknadel().subscribe(
      (data: any[]) => {
        if (this.map) {
